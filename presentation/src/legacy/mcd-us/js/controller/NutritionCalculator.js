...
app.controller('nutritionCalculatorController',['$scope','$q','$http','$cookies','getCoop','$timeout', function($scope, $q, $http, $cookies, getCoop, $timeout) {
    ...
    var dynamicStagingUrl = "";
    var dynamicStagingUrl2 = "http://dna.staging.mcdonalds.com";
    ...
    $scope.addItem = function(id,prevent,e) {
      console.log('add item');
          ...
          console.log($scope.itemsJsonResponse.length);
          ...
          console.log('going into else');
            ...
            console.log('went into items details');
            		var itemObj = {};
    ...
    $scope.updateItems = function(id,time,e) {
      ...
      console.log('updateItems');
    	//$(window).scrollTop($('#level-details'));
    	$http.get(dynamicStagingUrl+itemDetailsUrl+"&item=" + id).then(function(response) {
        		...
        		if ($scope.itemsJsonResponse[i].time==time) {
        			...
        			if ($scope.sizeChanged) {
        				if ($scope.itemsJsonResponse[i].has_components=='No') {
        					$scope.updateNutritionValues(true);
        					$scope.sizeChanged = false;
        				}
        	    	}
    ...
    $scope.isComponentChecked = function(id) {
      console.log('isComponentChecked');
        if (id) {
    ...
    $scope.getCatDetails = function() {
        ...
        $scope.catItems = [];
        $http.get(dynamicStagingUrl+catDetailsUrl+"&categoryId=" + $scope.catId+coopurl).then(function(response) {
            $scope.categoryLoading = false;
    ...
    $scope.updateNutritionValues = function(scrollToSummary) {
    	...
    	$scope.myMealCounter = angular.element(".meal-item .meal-row > input:checked").length;
    	  ...
    	  if (str!="") {
    		  $http.get(dynamicStagingUrl+itemListUrl+"&item=" + str).then(function(response) {
    ...
    if ($cookies.get("MyMealItems")) {
      console.log('got cookies');
        ...
        var promise = $http.get(dynamicStagingUrl+itemDetailsUrl+"&item=" + array[i]).then(function(response) {
          var itemObj = {};
          itemObj.id = response.data.item.item_id;
          var itemTime = new Date().getTime();
          itemObj.time = itemTime;
...
app.directive('onFinishRenderCategory', ['$timeout',function ($timeout) {
  console.log('inside onFinishRednerCategory');
  return {
    restrict: 'A',
...
app.directive('onFinishRenderNc', ['$timeout',function ($timeout) {
  console.log('inside onFinishRenderNc');
  return {
    restrict: 'A',
    link: function (scope, element, attr) {
      if (scope.$last === true) {
...
app.directive('onFinishRenderNcProducts', ['$timeout',function ($timeout) {
  console.log('inside onFinishRenderNcProducts');
  return {
    restrict: 'A',
    link: function (scope, element, attr) {
      if (scope.$last === true) {
