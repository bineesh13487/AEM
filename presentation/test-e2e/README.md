...
Install the dependencies (`yarn install`) and then run `yarn test:e2e`

_please note_ The WDIO runner uses the configuration file `wdio.conf.js` by default.

## What we have in place?

- Full integration with WebdriverIO http://webdriver.io/ (http://webdriver.io/)
- Over 150 predefined steps that cover almost everything QA needs for integration tests
- Integration with Sauce Labs - Planning

# How to write a test?

Tests are written in [Gherkin syntax](https://cucumber.io/docs/reference) that means that you write down what's supposed to happen in a real language. All test files are located in `./test-e2e/features/*` and have the file ending `.feature`. You will already find some test files in that directory. They should demonstrate, how tests could look like. Just create a new file and write your first test.

***Look into the "features.sample" folder to learn by example***
...
For more information on writing tests scroll down to section *List of predefined steps*
...
You can setup multiple configs for specific environments. Let's say you want to have a different `baseUrl` for your local and pre-deploy tests. Use the `wdio.conf.js` to set all general configs (like mochaOpts) that don't change. They act as default values. 

For each different environment we need create a new config with the following name scheme:
...
We could create a specific config for your pre-deploy tests:
...
The predefined snippets allow you to do a lot of common things but you might need extra snippets which are better aligned with your aims. To do so you will find all step definitions in `./test-e2e/steps`. They are separated in `given`, `when` and `then`.
...
To assert values we are using [Chai](http://chaijs.com/) integration.
