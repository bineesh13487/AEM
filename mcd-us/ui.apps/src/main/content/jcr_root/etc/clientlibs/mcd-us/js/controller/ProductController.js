...
(function () {
  ...
  ProductController.$inject = ['$scope', '$cookies', '$http'];
  function ProductController($scope, $cookies, $http) {
    const itemDetailsUrl = $('.nutritionDnaUrl').data('itemdetails');
    ...
    $scope.updateItems = function (id, updateSize, e) {
      ...
      $http.get(`${itemDetailsUrl}&item=${id}`).then(response => {
        if (updateSize) {
    ...
    $scope.getDv = function (x) {
      const dvtext = $('.nutritionData').data('dvtext').toString();
      ...
      if (b.item.relation_types) {
        if (b.item.relation_types.relation_type !== undefined) {
          if (Array.isArray(b.item.relation_types.relation_type)) {
            const m = b.item.relation_types.relation_type;
            for (let i = 0; i < m.length; i++) {
              const elem = m[i];
              if (elem && elem.type !== 'master child') {
                const elem1 = elem.related_items.related_item;
                if (elem1) {
          ...
          else if (b.item.relation_types.relation_type.type !== 'master child') {
            const elem = b.item.relation_types.relation_type;
            if (Array.isArray(elem.related_items.related_item)) {
              for (let i = 0; i < elem.related_items.related_item.length; i++) {
                const elem1 = elem.related_items.related_item[i];
    ...
    $scope.getIngredients = function (b) {
      ...
      if (b.item.mutex_groups) {
        if (Array.isArray(b.item.mutex_groups.mutex_group)) {
          const m = b.item.mutex_groups.mutex_group;
          for (let i = 0; i < m.length; i++) {
            if (Array.isArray(m[i].constituents.constituent)) {
              const groupObj = {};
              for (let j = 0; j < m[i].constituents.constituent.length; j++) {
                const elem = m[i].constituents.constituent[j];
                if (elem) {
                  if (elem.is_default) {
          ...
          if (Array.isArray(m.constituents.constituent)) {
            for (let i = 0; i < m.constituents.constituent.length; i++) {
              const elem = m.constituents.constituent[i];
              if (elem) {
                if (elem.is_default) {
                  if (elem.id.toString().startsWith('2')) {
      ...
      if (b.item.components) {
        if (b.item.components.component) {
          if (Array.isArray(b.item.components.component)) {
            const m = b.item.components.component;
            for (let i = 0; i < m.length; i++) {
              const elem = m[i];
              if (elem) {
                const groupObj = {};
      ...
      const byDispOrder = groupObjList.slice(0);
      byDispOrder.sort((a, c) => a.display_order - c.display_order);
      $scope.pcIngredients = byDispOrder;
    };

    $scope.addToNc1 = function () {
      const ids = $scope.itemId;
      if (ids !== null && ids !== '') {
