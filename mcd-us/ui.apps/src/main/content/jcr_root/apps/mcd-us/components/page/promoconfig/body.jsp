...
<%
	...
	String contentPath = currentPage.getPath();
    String dlgPath = resource.getPath() + "/dialog";
	String templatePath = "/apps/mcd-us/templates/promotemplate";
	...
	Page parentPage = pageManager.getPage(contentPath);
    if(parentPage !=null){
        String[] templateTypes = {"promo", "carousel", "offer"};
        for(String templateType : templateTypes) {
            Page configPage = pageManager.getPage(contentPath+"/" + templateType);
            if(configPage == null){
                configPage =pageManager.create(contentPath,templateType,"/apps/",templateType);
            }
            Calendar today = Calendar.getInstance();
            String year = ""+today.get( Calendar.YEAR );
            Page yearPage = pageManager.getPage(configPage.getPath()+"/"+year);
            if(yearPage == null){
                yearPage =pageManager.create(configPage.getPath(),year,"/apps/",year);
            }
            String month =""+ new Integer(today.get( Calendar.MONTH ) + 1);
            Page monthPage =pageManager.getPage(yearPage.getPath()+"/"+month);
            if(monthPage == null){
                pageManager.create(yearPage.getPath(),month,"/apps/",month);
            }
        }
    }
    ...
    CQ.Ext.onReady(function() {
        ...
        myForm.addButton({
            ...
            handler: function() {
                ...
                var templateType = frm.findField("./jcr:content/templatetype");
                var dateObj = new Date();
                var year = dateObj.getUTCFullYear();
                var month = dateObj.getUTCMonth()+1;
                frm.url=parentPath+'/';
                frm.url=frm.url.substring(0,(frm.url.length))+templateType.getValue()+'/'+year+'/'+month+'/*';
